...
class MadaraPlugin implements Plugin.PluginBase {
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    ...
    if (this.options?.customJs) {
      try {
        chapterText.find('p').first().next('div').remove()
      } catch (error) {
